...
#define ESP01SOFTAPSSIDH		"ESP"
#define ESP01SOFTAPPASSWORD		""

#define ESP01SOFTAPIP			"192.168.13.1"
#define ESP01SOFTAPGATEWAY		"192.168.13.1"
#define ESP01SOFTAPMASK			"255.255.255.0"
 ...
 *
 */
void ESP01_SetWIFI(const char *ssid, const char *password, _eESP01MODE aMode);


/**
 * @brief ESP01_StartServerTCP Configura un servidor TCP
 *
 ...
 *
 */
_eESP01STATUS ESP01_StartServerTCP(uint16_t serverPORT);
 ...
 *
 * void ESP01DoCHPD(uint8_t value){
 *	HAL_GPIO_WritePin(CHDW_GPIO_Port, CHDW_Pin, value);
 * }
 *
 * int ESP01WriteUSARTByte(uint8_t value){
 ...
 * 		ESP01_StartUDP("192.168.xxx.xxx", 30000, 30000);
 *
 *		while(1){
 *			if(irTX != iwTX){
 *		  		if(irTX < iwTX)
 ...
 *			  		aux8 += iwTX;
 *		  		}
 *		  		if(ESP01_Send(bufTX, irTX, aux8, SIZEBUFTX) == ESP01_SEND_READY)
 *			  		irTX = iwTX;
 *	  		}
 *
 * 			ESP01_TASK();
